...
PHP_METHOD(SSDB, geo_neighbour) {
	...
	if (!ssdb_geo_neighbours(ssdb_sock, key, key_len, member_key, member_key_len, radius_meters, return_limit, offset_limit, zscan_limit, INTERNAL_FUNCTION_PARAM_PASSTHRU)) {
		RETURN_NULL();
	}
}
